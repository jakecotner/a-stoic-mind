...
from app.schemas import ReadingPageOut, TocOut, TocSection, WorkOut
...
# Cap synthesis *misses* per IP: cache hits are free and unlimited, but a miss
# calls a paid TTS API. The corpus being finite already bounds total spend at
# "whole corpus, once" — this just slows an abusive crawl to a trickle.
_MISS_LIMIT = 30
_MISS_WINDOW = 3600.0
_miss_log: dict[str, list[float]] = {}
_miss_lock = threading.Lock()

_AUDIO_CACHE_HEADERS = {"Cache-Control": "public, max-age=31536000, immutable"}


def _check_miss_cap(request: Request) -> None:
    ip = request.headers.get("x-forwarded-for", "").split(",")[0].strip() or (
        request.client.host if request.client else "unknown"
    )
    now = time.monotonic()
    with _miss_lock:
        recent = [t for t in _miss_log.get(ip, []) if now - t < _MISS_WINDOW]
        if len(recent) >= _MISS_LIMIT:
            raise HTTPException(429, "Too many narration requests; try again later")
        recent.append(now)
        _miss_log[ip] = recent


@router.get("/passages/{passage_id}/audio")
def passage_audio(passage_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    if row is None:
        ...
        _check_miss_cap(request)
        data, media_type = synthesize(passage.text)
        row = PassageAudio(
