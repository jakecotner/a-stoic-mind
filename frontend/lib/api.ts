...
// --- Orgs (optional orgs/RBAC module — mirrors backend app/routes/org.py +
// app/services/org.py; delete together, see MODULES.md)

export type Org = Schema<"OrgOut">;
export type OrgMember = Schema<"MemberOut">;
export type OrgInvite = Schema<"InviteOut">;
export type OrgRole = Org["role"];
export type InviteRole = OrgInvite["role"];

/** Readable message from a structured error detail ({code, message}). */
async function detailMessage(resp: Response, fallback: string): Promise<string> {
  try {
...
export async function fetchOrgs(): Promise<Org[]> {
  ...
  if (!resp.ok) throw new Error(`Could not load organizations (${resp.status})`);
  return resp.json();
}
...
export async function createOrg(name: string): Promise<Org> {
  ...
  if (!resp.ok)
    throw new Error(
      await detailMessage(resp, "Could not create the organization"),
    );
  return resp.json();
}
...
export async function deleteOrg(orgId: string): Promise<void> {
  ...
  if (!resp.ok)
    throw new Error(
      await detailMessage(resp, "Could not delete the organization"),
    );
}

export async function fetchOrgMembers(orgId: string): Promise<OrgMember[]> {
  const resp = await fetch(`/api/orgs/${orgId}/members`);
  if (!resp.ok) throw new Error(`Could not load members (${resp.status})`);
  return resp.json();
}
...
export async function changeMemberRole(
  ...
  if (!resp.ok)
    throw new Error(await detailMessage(resp, "Could not change the role"));
  return resp.json();
}
...
export async function removeMember(
  ...
  if (!resp.ok)
    throw new Error(await detailMessage(resp, "Could not remove the member"));
}

export async function leaveOrg(orgId: string): Promise<void> {
  const resp = await fetch(`/api/orgs/${orgId}/leave`, { method: "POST" });
  if (!resp.ok)
    throw new Error(
      await detailMessage(resp, "Could not leave the organization"),
    );
}

export async function fetchOrgInvites(orgId: string): Promise<OrgInvite[]> {
  const resp = await fetch(`/api/orgs/${orgId}/invites`);
  if (!resp.ok) throw new Error(`Could not load invites (${resp.status})`);
  return resp.json();
}
...
export async function createOrgInvite(
  ...
  if (!resp.ok)
    throw new Error(await detailMessage(resp, "Could not send the invite"));
  return resp.json();
}
...
export async function revokeOrgInvite(
  ...
  if (!resp.ok)
    throw new Error(await detailMessage(resp, "Could not revoke the invite"));
}

export async function acceptOrgInvite(token: string): Promise<Org> {
  const resp = await fetch("/api/orgs/accept-invite", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ token }),
  });
  if (!resp.ok)
    throw new Error(
      await detailMessage(resp, "Could not accept the invitation"),
    );
  return resp.json();
}
